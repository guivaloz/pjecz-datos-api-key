...
git clone https://github.com/PJECZ/pjecz-datos-api-key.git
cd pjecz-datos-api-key
```
...
```bash
podman build -t pjecz_datos_api_key .
```
...
```bash
podman container stop pjecz_datos_api_key
```

Arrancar contenedor

```bash
podman container start pjecz_datos_api_key
```

Eliminar contenedor

```bash
podman container rm pjecz_datos_api_key
```

Eliminar la imagen

```bash
podman image rm pjecz_datos_api_key
```
